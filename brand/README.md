...
Marin's visual identity rules — the source of truth for what the County brand *is*, distinct from how it's implemented as reusable interface tokens (that's `marin-ui/tokens`).
...
Distilled from `marinskills/brand-standards` (in full) and the MarinOS-specific identity bits from its "MarinOS web standards" section — the component/interaction rules in that same section (menu behavior, card content rules, nav reset) stayed in place for the upcoming product-design migration pass, since they're interaction patterns, not visual identity.
